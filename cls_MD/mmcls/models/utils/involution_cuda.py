...
class _involution(Function):
    ...
    def forward(ctx, input, weight, stride, padding, dilation):
        ...
        # output_h = int(torch.true_divide((height + 2 * padding[0] - (dilation[0] * (kernel_h - 1) + 1)) , stride[0] + 1))
        # output_w = int(torch.true_divide((width + 2 * padding[1] - (dilation[1] * (kernel_w - 1) + 1)),stride[1] + 1))
...
class My_involution(nn.Module):
    ...
    def __init__(self,
                 ...
                 padding=3,
                group_channel=8):
        super(My_involution, self).__init__()
        self.kernel_size = kernel_size
        self.stride = stride
        ...
        self.groups = self.channels_out // self.group_channels
        #self.groups=groups
        self.conv1 = ConvModule(
            in_channels=channels_in,
            out_channels=channels_out // reduction_ratio,
            kernel_size=1,
            conv_cfg=None,
            norm_cfg=dict(type='BN'),
            act_cfg=dict(type='ReLU'))
        self.conv2 = ConvModule(
            in_channels=channels_out // reduction_ratio,
            out_channels=kernel_size ** 2 * self.groups,
            kernel_size=1,
            stride=1,
            conv_cfg=None,
            norm_cfg=None,
            act_cfg=None)
        self.initial_mapping = nn.Conv2d(in_channels=self.channels_in, out_channels=self.channels_out,
    ...
    def forward(self, x):
        ...
        b, c, h, w = weight.shape
        weight = weight.view(b, self.groups, self.kernel_size, self.kernel_size, h, w)
        out = _involution_cuda(self.initial_mapping(x), weight, stride=self.stride, padding=(self.kernel_size - 1) // 2)
        return out
